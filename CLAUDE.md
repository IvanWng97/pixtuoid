...
- **A refuted review finding produces a MECHANISM, or nothing** — a test, a compile-time constraint, or a CI gate; refuting never produces prose, because prose has no failure mode. Choose nothing when the failure would show without one — CI turns red, a release exposes it, or it only dims an advisory signal: a mechanism is for a failure that stays silent and costs something, and every one it adds is another contract to keep in step. Only an EXTERNAL fact (another CLI's wire bytes, an OS semantic) earns a comment, on the declaration it constrains. **A real finding this change introduced is fixed in-scope or forces a re-scope; a pre-existing one is SURFACED to the owner in one line (four terminal states, defined once in `pr-review.prompt.md`). Agents never file issues.**
