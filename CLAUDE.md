...
- **A claim about an EXTERNAL surface is fetched, not remembered.** Any assertion that an upstream file does or does not contain something — in a comment, a doc, a commit message, or a review reply — either cites the `path:line` you fetched THIS session or becomes a `check_upstream_drift.py` row. "No document declares it" and "that only appears in a comment" are the two spellings this repo has shipped wrong: #938 asserted both about grok's `_x.ai/session/update`, having read one file of the twenty that name it, and used each to justify deleting a check. Grepping ONE plausible file is the failure, not laziness — the population is the whole upstream repo, and `gh api .../git/trees/<ref>?recursive=1` enumerates it.
...
6. **Walkable mask = ground footprint only.** Visual sprites can be wider than their footprint; the mask blocks only the ground-level projection, so characters walk right next to walls. The dependency runs ONE way: sprite size never moves a sim position — fitting a frame on the canvas is the painter's (`keep_sprite_on_canvas`), and clamping the SIM's answer instead is what put a SIM position inside furniture (#912).
