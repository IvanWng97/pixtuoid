...
- **No comments unless WHY.** Comment only what a future reader can't tell from the code (a workaround, a non-obvious constraint, a surprising invariant). Three tests, in order — the first two are the known art, the third is the one this repo kept failing:
  1. **A different abstraction level than the code.** If the reader could deduce it from the line below, delete it (Ousterhout, *A Philosophy of Software Design* ch. 13 — "comments repeat the code" is the commonest way a comment is useless).
  2. **Every sentence carries information the earlier ones don't.** Rule 1 catches comment-vs-CODE redundancy; this catches comment-vs-ITSELF — one WHY restated from three more angles, which rule 1 cannot see because none of the restatements repeats the code. **The check: delete every sentence after the first. Would a reader now do something wrong?** No → they were restatement, cut them. Yes → that sentence is a *second* WHY and belongs on its own declaration, not appended to this one.
  3. **First sentence is the whole answer.** A reader who stops there is not misled.

  Evidence, not decoration: measurements, incident numbers and before/after counts belong in the COMMIT MESSAGE, where `git blame` surfaces them on demand and they cannot rot. Applied to one real `add_blind` docstring, these cut 11 lines to 2 with no information lost — the other 9 were one idea said four times and a second idea said twice.
