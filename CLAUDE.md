...
- Don't `git push` without explicit user confirmation, even after committing. (`git push` / `gh pr merge` — incl. the `rtk`-rewritten forms — are also surfaced as an `ask` via `permissions.ask` in `.claude/settings.json`, a Claude-Code confirm backstop for the common forms; env-prefixed / chained variants and `--no-verify` still rely on this prose + the human.)
