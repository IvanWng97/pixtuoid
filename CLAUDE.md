...
- **No comments unless WHY.** Comment only what a future reader can't tell from the code (a workaround, a non-obvious constraint, a surprising invariant). Three tests, in order — 1 and 3 are the known art; 2 is the one this repo kept failing:
  ...
  2. **Every sentence carries information the earlier ones don't.** Rule 1 compares the comment to the CODE; this compares it to ITSELF, which is why nothing else catches it. **The check: delete every sentence after the first. Would a reader now do something wrong, OR be unable to tell when this constraint stops applying?** No → restatement, cut it. Yes → it is a *second* WHY; move it to the declaration it actually constrains, if that is a different one. (Two independent WHYs about THIS declaration both stay, and a WHY about a statement stays with the statement.) The second half of the question is load-bearing: `// Never call this on the UI thread. It deadlocks on the render lock.` is still obeyed with sentence 2 deleted, and that is exactly how a comment rots into an unexplained taboo the next reader removes.
  3. **First sentence is the whole answer.** A reader who stops there is not misled and not missing anything actionable.

  Evidence, not decoration: in a code comment, a measurement or before/after count belongs in the COMMIT MESSAGE — an inline number nobody re-measures is the first thing to rot. An issue number STAYS when it names the failure mode the comment exists to prevent (`#793` in `check_upstream_drift.py`), and is not provenance. Don't reach for `git blame` as the retrieval path: it names whoever last touched the line, which after a reformat is not the author of the rationale.
