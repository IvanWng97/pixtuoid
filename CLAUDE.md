...
- Release: dispatch `release-plz.yml` → `just gen` on its PR → merge; that merge IS the publish (the `tag` job creates `vX.Y.Z`, which fires release.yml: crates.io + npm + homebrew autobump) and stays a human step. [`CONTRIBUTING.md`](docs/CONTRIBUTING.md#releasing).
