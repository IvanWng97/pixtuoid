...
- **A claim about an EXTERNAL surface is fetched, not remembered.** Any assertion that an upstream file does or does not contain something — in a comment, a doc, a commit message, or a review reply — either cites the `path:line` you fetched THIS session or becomes a `check_upstream_drift.py` row. "No document declares it" and "that only appears in a comment" are the two spellings this repo has shipped wrong (#938), each used to justify deleting a check. Grepping ONE plausible file is the failure, not laziness — the population is the whole upstream repo, and `gh api .../git/trees/<ref>?recursive=1` enumerates it.
