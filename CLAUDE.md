...
- **Only the latest released version of each agent CLI is supported** — no fallback arms, no dual-listening for an old event name beside its replacement, no legacy-format decoder kept "just in case". When upstream renames or reshapes, repoint the decoder, the plugin, and the drift-watcher anchor at the CURRENT declaration and delete the old one: every legacy arm is a second copy of a wire contract that drifts silently and that the watcher then has to anchor twice (#981).
