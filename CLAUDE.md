...
- Release: dispatch `release-plz.yml` → merge its PR (no `just gen` — nothing rendered carries the version); that merge IS the publish (the `release` job does crates.io + the tag + a draft release; the tag fires release.yml: binaries + npm + homebrew autobump) and stays a human step. [`CONTRIBUTING.md`](docs/CONTRIBUTING.md#releasing).
