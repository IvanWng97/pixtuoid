...
- `docs/{CONFIGURATION, ARCHITECTURE, CONTRIBUTING, MIGRATION,
  KNOWLEDGE-ENGINEERING}.md` → rendered as `/config`, `/architecture`,
  `/contributing`, `/migration`, `/knowledge-base` respectively, via a `glob`
  loader in `src/content.config.ts` + a `src/pages/*.astro` per route.
