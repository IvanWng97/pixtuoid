...
- The root `README.md` Features table + install commands are GENERATED from
  ...
  each row — the site-side consumer of the REST of the row (`card.blurb` /
  `card.href`) is now the merged 5F band inside `Showcase.astro` (the standalone
  `Features.astro` component is gone); a `card.href` of the form
  `#showcase-<id>` is guard-validated at build time against `showcase.json`
  (`astro.config.mjs`), so a feature row can never deep-link to a channel that
  doesn't exist.
- The six-floor anchor vocabulary (`data-floor="6F"`…`"1F"` + `data-floor-label`,
  stamped by **wb-1** from `consts.ts`'s `FLOORS`) is what `ElevatorShaft.astro`
  (mounted index-only, a `pix:paused` set member) and the Statusline's
  scrollspy both consume off the one `FLOOR_SPY_ROOT_MARGIN` band. The
  Showcase/Features merge (**wb-3**) re-keys the 4F `FLOORS` entry from
  `features` to `amenities` (mounting the `section#amenities` shell in
  `index.astro`; wb-4/wb-5 mount their content inside it), keeps a `#features`
  anchor-compat shim atop the merged 5F roster so inbound deep links still
  resolve, and adds `data-keys-scope="channels"` + scoped channel-tune keys on
  `Showcase.astro`'s `section#showcase` (the first live example of the
  focused-region carve-out the global digit handler already respects — see
  `README.md`'s cross-component-seams note). The inner studio-wall div keeps
  `id="studio"` — no collision, since the 5F section id is `#showcase`.
