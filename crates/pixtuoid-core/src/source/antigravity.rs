...
mod tests {
    ...
    /// only the previous step's primary end fires — and the drift `unknown_event`
    /// breadcrumb path (defense #2, the sole currency backstop for AG's
    /// schema-less format) is exercised on the way. Guards the else-branch
    /// restructure that added that breadcrumb without changing the event output.
    #[test]
    fn renamed_planner_step_with_tool_calls_safe_degrades_to_end_only() {
        let v = serde_json::json!({
        ...
        assert_eq!(
            out.len(),
            1,
            "a renamed tool-call step must mint NO start, only the prev end: {out:?}"
        );
        match &out[0] {
            AgentEvent::ActivityEnd { tool_use_id, .. } => {
                assert_eq!(tool_use_id.as_deref(), Some("ag-1-0"));
