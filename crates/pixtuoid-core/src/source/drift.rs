...
//! `copilot.payload_fields`, codex's escalation pair `codex.escalation`, and
//! omp's only human-readable root label rides `omp.title_fields`; the rest are
//! neither watched nor breadcrumbed)
