...
const DEFAULT_PROMPT: &str = "Read NOTE.txt, then list this directory, then read NOTE.txt again.";
...
fn place_transcript(fresh: &Path, candidates: &[PathBuf], dest: &Path) -> std::io::Result<PathBuf> {
...
fn run_cli(cmd: &[String], ws: &Path, sock: &Path) -> std::io::Result<std::process::ExitStatus> {
