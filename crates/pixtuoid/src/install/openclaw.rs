...
/// arms by the consistency test below, which is why a const with no prod caller
/// stays. A rename upstream makes that hook silently stop firing, since the
/// plugin registers by name.
#[cfg(test)]
