...
/// The OS-correct hook `command` string for a CLI whose hook runner SHELLS the
/// command (`/bin/sh -c` on Unix, `cmd.exe /C` on Windows) — Codex and Reasonix
/// both do. The single OS fork for that strategy, so a new cmd.exe-shelling CLI
/// pays zero platform cost.
///
/// - **Unix**: env-prefix form `PIXTUOID_SOURCE=<source> '<path>'` (single-quoted
///   for spaces).
/// - **Windows**: BARE exec form `<path> --source <source>` via
///   `windows_bare_hook_command` (cmd.exe can't express the env-prefix; the
///   source rides as the shim's `--source` flag, and a space/metacharacter path
///   falls back to its DOS 8.3 short name, rejecting only if 8.3 is disabled).
///
/// Claude is NOT a caller — it writes the exec form (absolute `.exe` + args), the
/// opposite strategy.
pub(crate) fn shell_hook_command(path: &str, source: &str) -> Result<String> {
    #[cfg(windows)]
    {
        windows_bare_hook_command(path, source)
    }
    #[cfg(unix)]
    {
        Ok(format!(
