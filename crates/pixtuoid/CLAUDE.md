...
│                         install-hooks CLI); has_hooks(t) is `pub(crate)` — its two callers are doctor::diagnose's
│                         verify gate and the onboarding-skip freeze (`sources::skip_freeze`, which probes it to keep a
│                         pre-0.12 upgrader's hooks); 0.12.0 dropped resolve_connected's install-state migrate inference),
