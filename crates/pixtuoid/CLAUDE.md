...
- "How does config persistence work?" → `config.rs` defines `AppConfig` (theme + optional max-desks cap + pack-dir + enabled-pets + pet-names), `config_path()` (XDG-aware: `$XDG_CONFIG_HOME/pixtuoid/config.toml` or `~/.config/pixtuoid/config.toml`), `load()` (never crashes — logs warning on malformed TOML), `save()` (advisory-locked atomic tmp+rename), `resolve_theme()` (CLI > config > default). Theme saved on `[t]` picker Enter confirm in `tui/mod.rs`. `max-desks` is an optional cap — when set, auto-compute clamps each floor's capacity to `min(layout_capacity, cap)`. When absent, fully auto-computed from terminal size. `pack-dir` supports `~` expansion via `resolve_pack_dir`. **`resolve_pets(&AppConfig) -> Vec<Pet>`** is the single pet resolver: it merges `enabled-pets` (kinds; absent = all) with `[pet-names]` (per-kind custom name; trimmed, empty → default) into `Vec<Pet>` where `Pet { kind, name }`. There is NO separate `resolve_pet_names` (it was folded in) and NO runtime kind→name map — the resolved name rides on each `Pet`, so the renderer reads `pet.name` directly. **`pet_names` MUST be the LAST field in `AppConfig`** — it serializes as a `[pet-names]` table and `toml::to_string_pretty` requires tables after all scalar keys (a scalar field declared below it emits invalid TOML).
