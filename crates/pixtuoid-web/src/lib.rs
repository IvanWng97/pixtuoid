...
mod tests {
    ...
    fn capacity_tracks_the_canvas_layout_so_no_agent_is_stranded_unpainted() {
        ...
        // narrow bufW): the scripted cast holds most desks by ~2.5s (the
        // morning rush walk-in). The reducer's capacity must derive from THAT
        // layout, so an admitted agent always has a paintable desk anchor —
        // an agent whose desk index falls off the canvas layout renders
        // NOWHERE (character_anchor returns None) while staying alive in the
        // scene. Free-desk count is DERIVED (capacity − cast), not a size
        // literal — the 2026-07-11 density pass proved pitch consts re-tune
        // hardcoded seat counts out from under this test.
        let (w, h) = (96u32, 100u32);
        let mut o = office();
        let mut t = 0u64;
        while t <= 30_000 {
        ...
        assert!(free >= 1, "the cast must leave spare desks for hires");
        ...
        // the MAX_LIVE slots for its whole stay with zero feedback. (Capped
        // by MAX_LIVE, which the 16:9 test below exercises.)
