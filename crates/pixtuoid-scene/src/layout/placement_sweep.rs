...
/// The strongest connectivity truth: every walkable pixel is reachable from the
/// door threshold by a 4-connected flood, and the threshold itself is walkable.
/// Extracted so BOTH the discrete `SWEEP_SIZES` pass and the narrow-band
/// step-1 boundary scan assert the same thing.
...
/// on the sole inter-pod drain at a single-pod-column band).
const NARROW_BAND: std::ops::RangeInclusive<u16> = 32..=64;
...
fn couch_survives_a_narrow_band_that_clears_the_door() {
    // Over-drop guard for the CLASS A gate: an over-conservative couch-east-edge
    // estimate would silently vanish the couch at widths that DO clear the door.
    // 48x160 seed 0 clears — the couch must stay (the boundary scan can't catch
    // an over-drop; dropping the couch only IMPROVES connectivity).
    let l = SceneLayout::compute_with_seed(48, 160, None, 0).expect("48x160 lays out");
    assert!(
        l.couch_sprite_center.is_some(),
        "the lounge couch must survive a band wide enough to clear the door threshold"
    );
}
