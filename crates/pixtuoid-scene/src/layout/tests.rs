...
#[test]
fn scatter_plants_never_visually_stack_on_waypoint_obstacles() {
    // Film-critic catch (#562 audit, pre-existing): the aisle-corner Flower
    // sat 1px above the VendingMachine in the SAME column — geometrically
    // disjoint (grounds fine, sweep silent) but the machine's red panel row
    // reads as part of the bouquet: a totem. The invariant is therefore
    // CLEARANCE, not mere non-overlap: a scatter plant's sprite box keeps
    // PLANT_APPLIANCE_CLEARANCE_PX of air from any obstacle waypoint's box.
    ...
    let inflate = |(tl, sz): (Point, Size), m: u16| {
        (
            Point {
            ...
            for wp in &l.waypoints {
                ...
                assert!(
                    !super::placement::rects_overlap(
                        visual_tl(p.pos, pv),
                        inflate(
                            visual_tl(wp.pos, def.visual),
                            super::compute::PLANT_APPLIANCE_CLEARANCE_PX
                        )
                    ),
                    "{w}x{h}: plant {:?} at {:?} visually stacks on {:?} at {:?}",
                    p.kind,
                    p.pos,
                    wp.kind,
                    wp.pos
