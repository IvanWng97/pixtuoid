...
/// The painter's half of the split #912 forced: the sim hands over a walkable
/// point untouched by sprite size, and keeping the drawn sprite on the canvas is
/// this layer's job — the one that may move a point, because moving a DRAWN
/// position harms nothing.
#[test]
fn the_painter_keeps_every_mascot_frame_on_the_canvas() {
    use crate::creatures::{mascot_position, mascot_seed};
    ...
    for port in ["18901", "18902", "18903", "18904", "100", "805"] {
        ...
        for step_ms in (0..36_000u64).step_by(120) {
            let age = 5_000 + step_ms;
            let now = t0 + std::time::Duration::from_millis(age);
            let presence = idle_presence_at(now, age);
            ...
            // The painter's own call, not a second copy of its arithmetic.
            let drawn = keep_sprite_on_canvas(pos, fw, fh, w, h);
            let (x0, y0) = (
                drawn.x.saturating_sub(fw / 2),
                drawn.y.saturating_sub(fh / 2),
            );
            checked += 1;
            assert!(
                x0 + fw <= w && y0 + fh <= h,
                "port {port} at +{step_ms}ms: {fw}x{fh} sprite drawn at {drawn:?} spills the \
                 {w}x{h} canvas (rect {x0},{y0}..{},{})",
                x0 + fw,
                y0 + fh
            );
        }
...
fn idle_presence_at(now: SystemTime, age_ms: u64) -> pixtuoid_core::state::DaemonPresence {
