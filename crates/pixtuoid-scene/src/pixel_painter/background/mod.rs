...
pub(super) fn paint_zone_rug(buf: &mut RgbBuffer, room: crate::layout::Bounds, rug: Rgb) {
    ...
    for y in y0..y1.min(buf.height()) {
        for x in x0..x1.min(buf.width()) {
            ...
            // WHY 45/65: 45% keeps carpet texture visible through the tint;
            // the 65% edge band draws the rug border (taste-pinned 2026-07-11).
            let w = if edge { 65u16 } else { 45 };
            buf.put(
                x,
                y,
                Rgb {
                    r: ((c.r as u16 * (100 - w) + rug.r as u16 * w) / 100) as u8,
                    g: ((c.g as u16 * (100 - w) + rug.g as u16 * w) / 100) as u8,
                    b: ((c.b as u16 * (100 - w) + rug.b as u16 * w) / 100) as u8,
                },
            );
        }
    }
...
pub(super) fn paint_wall_dressing(
    buf: &mut RgbBuffer,
    buf_w: u16,
    top_wall_h: u16,
