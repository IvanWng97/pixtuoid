...
// Temporary #900 measurement harness (the issue's own methodology): release
// only, `cargo test -p pixtuoid-scene --release perf_measure_900 -- --ignored --nocapture`.
#[test]
#[ignore = "manual perf measurement for #900"]
fn perf_measure_900() {
    ...
    let idle = make_scene(12, 16);
    let mut busy = make_scene(12, 16);
    let kinds = [
        ToolKind::Bash,
        ToolKind::Edit,
        ToolKind::Read,
    ...
    for slot in busy.agents.values_mut() {
        let i = slot.desk_index.0;
        slot.state_started_at = base + Duration::from_secs(i as u64);
        slot.last_event_at = base + Duration::from_secs(i as u64);
        slot.state = match i {
        ...
        slot.tool_call_count = i as u32 * 7;
    }

    for (scene_label, scene) in [("idle", &idle), ("busy", &busy)] {
        for (w, h) in [(192u16, 160u16), (360, 240)] {
            let mut fctx = FloorCtx::new();
            let mut buf = RgbBuffer::filled(0, 0, pixtuoid_core::sprite::Rgb { r: 0, g: 0, b: 0 });
            let mut coffee = CoffeeState::new();
            let mut chitchat = HashMap::new();
            let mut frame = |fctx: &mut FloorCtx, buf: &mut RgbBuffer, i: u32| {
                render_floor(
                    fctx,
                    buf,
                    &mut coffee,
                    ...
                    FrameInputs {
                        ...
                        pack: &pack,
                        theme,
                        now: base + Duration::from_millis(u64::from(i) * 100),
                        size: Size { w, h },
                        floor_meta: FloorMeta::ground(),
                        active_pet: None,
                        floor_pet: None,
            ...
            for i in 0..50 {
                frame(&mut fctx, &mut buf, i);
            }
            let t0 = Instant::now();
            for i in 0..FRAMES {
                frame(&mut fctx, &mut buf, 50 + i);
            }
            let whole = t0.elapsed() / FRAMES;
            ...
            for _ in 0..FRAMES {
                for _ in 0..2 {
                    let snap = buf.clone();
                    crate::pixel_painter::wash_since(&mut buf, &snap, wash);
                }
                black_box(&buf);
            }
            let scans = t1.elapsed() / FRAMES;

            println!(
                "{scene_label} {w}x{h}: avg frame {:?} ({:.0} fps equiv), \
                 2 clones + 2 scans (no writes) {:?} ({:.1}%)",
                whole,
                1.0 / whole.as_secs_f64(),
                scans,
                100.0 * scans.as_secs_f64() / whole.as_secs_f64()
            );
        }
    }
}
