...
├── creatures.rs    ambient wandering-creature BEHAVIOR (office pet + …
...
- Agent OUTFIT (shirt+pants) is keyed on the normalized `cwd`, not `agent_id`
- `recolor_frame` substitutes by RGB equality.
- EXIT walks are time-compressed to fit the GC window; entry/wander/snap-back are not.
- A walk leg's A\* polyline shape is frozen once per leg, not re-routed per frame.
- A meeting room narrower than `MEETING_FURNITURE_MIN_W` (compute.rs) has NO sofa/table/seats — bare floor, BY DESIGN.
- Occlusion is EMERGENT — there is no `occludes_behind` field / synthetic cap any more (deleted).
- Pantry counter blocks only a shallow `PANTRY_FOOTPRINT_DEPTH` south strip, not its full sprite height.
- splitmix64 is open-coded FOUR times in this crate on purpose
- `paint_glass_wall_h` and `paint_glass_wall_v` (`pixel_painter/wall.rs`) stay SEPARATE — don't hoist a `paint_glass_strip(axis, alphas)` helper.
- `epoch_ms(now)` is the ABSOLUTE wall-clock ms (~1.7e12) — reduce it BEFORE any `as f32`.
- **The sun/moon disc keeps a "real low window":** visible only at LOW altitude, absent at solar midday / lunar midnight.
- **Day-over-night light invariant:** the moon casts no direct beam, and any solar noon outshines the brightest night.
- The weather VEILS are lit by the emitter, and their cross-weather ordering is NOT an invariant.
- An EXCLUSIVE spot is single-occupancy, enforced where the destination is CHOSEN — not where it's drawn.
- Two narrow-band connectivity guards keep the office ONE region (#566), both graceful DECOR degradation — not bugs.
- Every wander destination is filtered by `ReachSet`, but the ±4px `jitter_dest` perturbation is applied AFTER that filter — a small residual.
