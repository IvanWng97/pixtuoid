...
- **The size gate cannot be replaced by a search over the real placer — below it the layout PANICS, not just misbehaves.** `compute_with_seed`'s early `None` is also the precondition guard for ~900 lines: at `buf_h` …
