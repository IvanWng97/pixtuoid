...
- **The GENERATED night pad renders NO sub-bass — lane 5 (the BASS stem) owns that register; only the frozen v4 night-pad anchor still bakes its sub into the pad.** `night_pad_core` takes `sub_gain`: the `#[cfg(test)]` anchor passes `NIGHT_PAD_SUB_GAIN` (3.2 …
<!-- edges:end -->
