...
- **The painter's canvas nudge can seat a creature ON furniture — the accepted side of a real conflict, not a leftover of #912.** `keep_sprite_on_canvas` moves a sprite inward by the bounds its `Anchor` implies — `Center` for …
- **A character is clamped to the canvas TWICE, against two DIFFERENT frame sizes — deliberately.** `sim::resolve_characters` clamps the sprite on the pack's real frame while …
<!-- edges:end -->
