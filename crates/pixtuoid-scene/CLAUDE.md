...
- **A desk chair's z key TIES with its occupant's and the order is decided by INSERTION, not by the key.** `enqueue_desk_chairs` runs after `enqueue_characters` and `sort_by_key` is stable, which is the whole mechanism putting the chair over the sitter — reorder those two calls, or swap in an unstable sort, and the chair silently paints UNDER them with every test still green. The tie is not a coincidence to break either: a seated character's key reduces to `desk_walk_anchor_facing(desk, facing).y` (`seated_anchor_facing` subtracts `WALKING_Y_OFF` from the walk anchor and `sim` adds it straight back), and the chair reads that same expression, so the two cannot drift apart. That cancellation has a PRECONDITION, because the subtraction saturates: a walk anchor within `WALKING_Y_OFF` of the top clamps to 0 and the character's key returns as `WALKING_Y_OFF`, keying the chair BELOW its occupant. No swept layout comes close (the measured floor is far above it), and `a_desk_walk_anchor_never_saturates_the_seated_z_key` is what keeps it that way — a shorter wall band or a desk grid moved north would otherwise slide chairs under their sitters with every render test green. Keying the chair off its OWN south row instead would sort it past the desk's `desk.y + visual.h` and paint it over the desk front. The chair belongs to the DESK, not the occupant — one per north-facing desk whether or not anyone is in it, so the office keeps its furniture when a session ends and the chair no longer slides sideways under a sitter still gliding into the seat.
