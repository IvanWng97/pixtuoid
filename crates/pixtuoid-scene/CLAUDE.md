...
`just release-e2e` walks them all without a second copy of the roster or of any
...
- **The painter's canvas nudge can seat a sprite ON furniture — the accepted side of a real conflict, not a leftover of #912.** `keep_sprite_on_canvas` moves a sprite inward by the bounds its `Anchor` implies — `Center` for …
