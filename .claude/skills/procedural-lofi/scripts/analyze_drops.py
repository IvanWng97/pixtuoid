...
fingerprint can't see: audible individual raindrops over the wash. Scans the
whole file for drop-event density, then zooms into the richest minute for the
per-drop rebuild targets (level-above-bed, centroid, decay).
"""
...
def detect_drops(x, sr):
    """Transient peaks well above the bed level."""
    env = band_env(x, sr)
    med = np.median(env)
    events = []
    thr = med * 3.0
