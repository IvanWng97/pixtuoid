...
3. **Verify** each finding adversarially (default REFUTE; read the comments
   on the item it names; construct a repro or refute) — a separate skeptic per finding, never the
...
| "The reviewer said X, so fix X" | Verify the premise first — read the item's comments; a wrong fix contradicts a design decision. |
...
| "Just unify the duplication" | Some duplication is documented deliberate separation (per-source decoders, per-CLI targets); read the item's comments before proposing a merge. |
