...
- **`pub fn <cli>_home()`** if the CLI has a custom config root — one fn honoring
  ...
  installer's `default_config_path()` so they can't disagree. **Audit that precedence against the
  CLI's OWN resolver per axis first** (#880; axis list in `CONTRIBUTING.md` step
...
- Capture the real wire shape and set `verified_version` (`"unknown"` until a
  ...
  `docs/CONTRIBUTING.md` "Adding a new agent CLI" (four edits, two of them
  silent), the row SHAPE in `scripts/check_upstream_drift.py`'s `SURFACE_ROWS`,
