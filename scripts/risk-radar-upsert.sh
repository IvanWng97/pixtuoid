...
# token that cannot write a comment never fails the job — the radar also goes to
# the step summary, which always works.
#
...
# A GET *failure* must NOT be mistaken for "no comment" — that would POST a
# duplicate sticky comment and break idempotency. The first marker'd id is the
# oldest, so it stays a stable target across re-pushes.
