...
The references are regenerated by scripts/gen-media.py (`just gen`) — a PR that
intentionally changes the office's look must commit them in the same change.
    ...
    for y in range(h):
        for x in range(w):
            if diff_data[x, y] != (0, 0, 0):
                mismatched += 1
                out_data[x, y] = (255, 0, 0)
            else:
                # Dim the background so the red overlay pops.
                c_pix = cand_data[x, y]
...
def _selftest():
    ...
    Two required gates ride on this predicate, so a silently-always-green
    comparator would report success for any render.
    """
    checks = []
