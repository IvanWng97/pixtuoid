...
| A CHECK, or its bound (a lint, gitleaks rule, census, threshold, probe) | Re-derive the bound YOURSELF from the incidents it cites, at the grain the check acts on, and enumerate the carriers it claims to cover. Then run it against the real historical bytes, per unit. A bound aggregated at the wrong grain (per-source where the check acts per-file) and a probe whose shape cannot exercise the rule both pass the author's own testing — six point-fixes in this tree exist because nobody re-derived (#995). |
