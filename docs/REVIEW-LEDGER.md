...
| R0613-08 | `source/codewhale.rs` `decode_cw_subagent` parent_id ↔ shim cwd-fallback | the subagent→parent link resolves only if the child's `workspace` string byte-matches the parent's shim-resolved cwd; under a no-`-C` launch the parent cwd comes from `current_dir()` (R0613-05) while the child carries CodeWhale's internally-resolved `workspace` (NOT through the shim) — they COULD diverge (trailing slash / symlink / abs-vs-rel), pointing parent_id at a non-existent slot | DEFERRED→issue #276 | genuinely unverifiable without a live subagent capture (the `-C` rig sets `DEEPSEEK_WORKSPACE` and bypasses the fallback; an automated pty can't reliably force a `subagent_spawn`) — the no-phasing rule's allowed defer class (no upstream signal). NOT a crash/data-loss: an unresolvable parent_id still registers the child as its own un-nested sprite, and workspace-absent is already a parentless root (`subagent_spawn_without_workspace_registers_a_parentless_root`). Speculative normalization REJECTED (could break a currently-matching pair without evidence). Found by the correctness lens (c55) | A | feat/codewhale-source | 2026-06-13 |
