...
## 2026-06-13 — Status panel + borderless popups review @ ad1655b (feat/status-panel-borderless-popups) → PR pending
...
The feature: an in-TUI Status panel (`c`) unifying hook install/uninstall + live
...
`StatusUi.selected` (usize-index soundness).

| # | seam (file/mechanism) | claim (1 line) | verdict | anchor (paths + fix) | tier | head | date |
|---|---|---|---|---|---|---|---|
| R0613-16 | `tui/status/mod.rs::build_rows` registry→target join via `install::target::by_name(d.name)` | the REGISTRY claude id is `claude_code::SOURCE_NAME = "claude-code"` but the install target name is `"claude"`, so `by_name("claude-code")` → None → the flagship CLI rendered as a non-actionable `JsonlNoHooks` row (i/u no-op); masked because the unit tests hand-fed `source_id:"claude"` and the snapshot fixture hardcoded the row. The impl-plan ASSUMED `SourceDescriptor.name == Target.name` (false for Claude) — a plan-miss | CONFIRMED→fixed | added `Target.core_source` (set from the core `SOURCE_NAME` consts, no drift) + `target::by_source`; `build_rows` now joins on `core_source`. Mechanized by `every_target_core_source_matches_a_registered_source` (target.rs) + `build_rows_makes_every_source_with_a_target_actionable` (status/tests.rs) — both fail on any future registry/target id drift. Found independently by the correctness AND design lenses (conf 96) | A | ad1655b | 2026-06-13 |
| R0613-17 | `scripts/media.json` — no `status` render job | the borderless Status panel shipped with zero rendered CI baseline (only `snapshot --status` existed, unwired), so a visual regression of the new flagship panel would pass gen-check silently | CONFIRMED→fixed | added a `status` render job (`snapshot --status` → `docs/images/status.png`, gen-check-gated), mirroring `dashboard`; baseline committed. Also moved the fixture's first row id `"claude"`→`"claude-code"` (production source id). Found by the design + film-critic lenses (conf 78) | A | ad1655b | 2026-06-13 |
