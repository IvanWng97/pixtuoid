...
// The target set lives in TWO hand-maintained places that can't share one
// literal (a YAML matrix and this JS table): release.yml's `build` matrix
// (what's compiled) and generate.mjs/this TARGETS (what npm packages). The
// dangerous drift is SILENT: add a build target but not to TARGETS → the
// artifact builds and npm ships the OLD set, no error. Pin both copies here
// (the repo's "can't centralize across a boundary → pin with a test" rule);
// the extract step self-guards (generate.mjs throws on a missing artifact).
//
// There is no longer a third copy: the self-hosted tap's formula was a
// desktop-only subset generated by a Ruby heredoc in release.yml, and both
// are gone — homebrew-core builds from the source tarball and carries no
// target list at all.
